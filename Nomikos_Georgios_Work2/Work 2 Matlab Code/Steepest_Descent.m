...
function [x_values, y_values, k, f_values] = Steepest_Descent(f, epsilon, x0, y0, gamma_choice, df_dx, df_dy, x, y, ConstantGamma)
    ...
    while norm([subs(df_dx, [x, y], [x_values(k), y_values(k)]), subs(df_dy, [x, y], [x_values(k), y_values(k)])]) >= epsilon
        

        d(k,1) = - subs(df_dx, [x, y], [x_values(k), y_values(k)]);
        d(k,2) = - subs(df_dy, [x, y], [x_values(k), y_values(k)]);
